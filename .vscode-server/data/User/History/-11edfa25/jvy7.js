...
async function loadWatchlist() {
    try {
        ...
        const emptyMessage = document.getElementById("empty-watchlist"); // ✅ Get the empty state message
        
        const watchedItems = watchlist.filter(item => item.watched);
        const unwatchedItems = watchlist.filter(item => !item.watched);
        
        if (watchedItems.length === 0 && unwatchedItems.length === 0) {
            emptyMessage.style.display = "block";  // ✅ Show empty message if BOTH lists are empty
        } else {
            emptyMessage.style.display = "none";   // ✅ Hide empty message if ANY list has items
        }
