...
async function loadWatchlist() {
    try {
        ...
        const emptyWatchedMessage = document.getElementById("empty-watched");

        // ✅ Clear existing items before appending new ones
        watchlistContainer.innerHTML = "";
        watchedContainer.innerHTML = "";

        // ✅ Separate watched & unwatched items
        const watchedItems = watchlist.filter(item => item.watched);
        const unwatchedItems = watchlist.filter(item => !item.watched);

        // ✅ Show empty messages based on individual lists
        emptyWatchlistMessage.style.display = unwatchedItems.length === 0 ? "block" : "none";
        emptyWatchedMessage.style.display = watchedItems.length === 0 ? "block" : "none";
