        ...
        watchlist.forEach(item => {
            ...
            div.innerHTML = `
                ...
                <div class="watchlist-actions">
                    <button class="menu-button" onclick="toggleMenu('${item.media_id}')">⋮</button>
                    <div class="menu-dropdown" id="menu-${item.media_id}">
                        <button onclick="toggleWatchedStatus('${item.media_id}', ${!item.watched})">
                            ${item.watched ? "Mark as Unwatched" : "Mark as Watched"}
                        </button>
                        <button onclick="removeFromWatchlist('${item.media_id}')">Remove</button>
                    </div>
                </div>
            `;
...
async function addToWatchlist(media_id, title, image, type) {
    ...
    try {
        const response = await fetch("/backend/watchlist", {
            ...
            body: JSON.stringify({ media_id, title, image, type }),
        });
...
async function removeFromWatchlist(media_id) {
    if (!checkAuth()) return; // Verify authentication

    const response = await fetch(`/backend/watchlist/${media_id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${localStorage.getItem('token')}` }
    });
...
async function toggleWatchedStatus(media_id, watched) {
    if (!checkAuth()) return; // Verify authentication
